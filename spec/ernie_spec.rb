...
describe Ernie do
  let(:ernie) { described_class.new(url_or_html, options) }
  ...
  describe '#to_pdf' do
    subject(:to_pdf) { ernie.to_pdf }

    let(:pdf_reader) { PDF::Reader.new pdf_io }
    let(:pdf_io) { StringIO.new to_pdf }
    let(:pdf_text_content) { Ernie::Utils.squish(pdf_reader.pages.first.text) }
    let(:large_text) { '<style>.text { font-size: 14px; }</style>' }
    let(:default_header) { Ernie::DEFAULT_HEADER_TEMPLATE }
    ...
    context 'when passing through options to Ernie' do
      subject(:to_pdf) { ernie.to_pdf }
      ...
      context 'when the page contains a line starting with `http`' do
        let(:options) { basic_header_footer_options.merge(header_template: large_text) }
        let(:url_or_html) do
          Ernie::Utils.strip_heredoc(<<-HTML)
            <html>
              <head>
  ...
  describe '#front_cover_path' do
    subject(:front_cover_path) { ernie.front_cover_path }
  ...
  describe '#back_cover_path' do
    subject(:back_cover_path) { ernie.back_cover_path }
  ...
  describe '#show_front_cover?' do
    subject(:show_front_cover?) { ernie.show_front_cover? }
  ...
  describe '#show_back_cover?' do
    subject(:show_back_cover?) { ernie.show_back_cover? }
  ...
  describe '#inspect' do
    subject(:inspect) { ernie.inspect }

    it { is_expected.to eq "#<Ernie:0x#{ernie.object_id} @url=\"http://google.com\">" }
  end

  describe '.configuration' do
    subject(:configuration) { described_class.configuration }

    it { is_expected.to be_a Ernie::Configuration }
  end
